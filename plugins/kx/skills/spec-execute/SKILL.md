...
**서브에이전트(개발/리뷰/테스트)를 호출하기 전 반드시 `~/.claude/agents/_spawn-rules.md`를 읽고 따를 것.** (`subagent_type` 명시, `run_in_background: false`, 결과 누락 시 즉시 보고 + 폴백)
...
2. context.md 상태 업데이트 — 갱신이므로 `## 갱신 이력`에 `- YYYY-MM-DD: 변경 요약`을 오늘 날짜로 한 줄 추가한다
