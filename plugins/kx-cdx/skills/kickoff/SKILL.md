...
이 커맨드는 작업 내용을 입력받아 **Codex planning workflow로 계획을 세우고**, 생성된 `plan.md`를 기반으로 `spec.md`를 역추출합니다.
...
3. **기존 문서 우선 탐색**: `specs/` 폴더를 탐색하여 관련된 폴더가 있는지 확인하십시오.
   - 없으면: `specs/[폴더명]/` 폴더를 생성하십시오.

4. 관련된 기능의 폴더가 이미 존재하면:
...
## 2. Planning Workflow 진입

**Codex에서 planning workflow를 실행하십시오.**

Planning workflow에서 다음을 수행:
...
## 3. spec.md 역추출 (planning workflow 완료 후)

planning workflow가 완료되면:
...
```
Documents created:
  - specs/[폴더명]/plan.md (구현 계획) - from planning workflow
